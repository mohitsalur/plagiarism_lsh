...
def _make_minhash_sigmatrix(shingled_data, num_hashes, inverted=False):
    ...
    for s, docid in inv_index:
        ...
        if s != last_s:
            hashvals = [hash_func(s) for hash_func in hash_funcs]
            last_s = s
        
        # Get the index of the document in the docids list
        doc_index = docids.index(docid)
        
        # Update the signature matrix with the minimum hash values
        for i in range(num_hashes):
            if hashvals[i] < sigmat[i, doc_index]:
                sigmat[i, doc_index] = hashvals[i]
...
class MinHash:
    ...
    def get_similarity(self, di, dj):
        ...
        # Compute the fraction of hash functions where the signatures match
        matches = np.sum(self._mat[:, i] == self._mat[:, j])
